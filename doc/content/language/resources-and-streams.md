---
kind: guide
id: guide.languageResourcesAndStreams
title: Resources and streams
summary: Proposes typed native handles, deterministic cleanup, and capability-based streams for THP.
nav:
  section: language
  order: 190
status: experimental
...
notice: >-
  Memory, temporary, URI, and read-only file streams now execute with shared
  cursor/close state, capabilities, typed exceptions, `using`, and PHPT
  coverage. Temporary streams spill through an unnamed host temporary file
  after their configured memory threshold.
---
...
The initializer's static type must nominally implement `Closeable`; merely
declaring compatible `close()` and `isClosed()` methods is not sufficient. The
binding is visible only inside the block. Its `close()` method runs exactly
once when control leaves the block by fallthrough, `return`, `break`,
`continue`, or an exception. Multiple resources use nested `using` blocks and
therefore close in reverse nesting order.
...
using ($value = new StructuralOnly()) {} // Compile error: not Closeable.
```
...
`php://temp/maxmemory:N`. It also recognizes the request-bound
`thp:/input` stream. Unknown schemes and malformed wrapper options throw
`InvalidStreamUriException`. User-registered wrappers, network schemes,
standard descriptors, output wrappers, and `php://filter` are reserved for
later proposals.
...
### Request input and host output

`Streams::open("thp:/input", OpenMode::Read)` returns the request's read-only,
non-seekable binary body stream. Every open in one request aliases the same
cell and cursor; closing any alias closes the shared stream. Other modes throw
`UnsupportedStreamOperationException`.

The SAPI body and PHPT `--STDIN--` are subject to
`request.post_max_size` and `time.max_input`. A known oversized body is
rejected before execution. An unknown-length body fails the read that crosses
the size limit without returning that chunk. Input-policy, heap, stack, and
handle failures terminate the request and are not catchable as THP
exceptions.

Program output does not accumulate in the request heap. `echo`, `var_dump()`,
the VM, and the JIT write synchronously to the host output sink, whose writes
provide backpressure. THP defines no total output quota. A host write failure
terminates the request; bytes already accepted by the host are not rolled
back. Embedders and the PHPT runner may opt into the explicit captured-output
wrapper.
...
- [Streams API reference](thp:std.streams)
